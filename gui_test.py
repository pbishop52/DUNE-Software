    ...
    def run(self):
        for voltage_stage in self.testing_process.standardTest():
            for relay in range(8):
                ...
                self.testing_process.measure_relay(relay)
...
class MainWindow(QWidget):
    ...
    def start_test(self):
        ...
        self.plot_data = []
        self.testing_thread = TestingThread(arduino_port, dmm_port)
    ...
    def update_plot(self):
        if self.is_testing:
            import random
            self.plot_data.append((len(self.plot_data), random.uniform(0, 10)))
